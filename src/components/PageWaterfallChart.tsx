...
function WaterfallTooltip({ active, payload }: { active?: boolean; payload?: Array<Record<string, unknown>> }) {
...
export function PageWaterfallChart({ activePages, inactivePages }: PageWaterfallChartProps) {
  const funnelCount = activePages.filter(p => p.activity_kind === 'funnel_converting').length;
  const chatCount = activePages.filter(p => p.activity_kind === 'chat_only').length;
  const directCount = activePages.filter(p => p.activity_kind === 'direct_orders_only').length;
      ...
      <div className="flex-1 w-full min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: -20 }}>
            ...
            <Tooltip content={<WaterfallTooltip />} cursor={{ fill: '#1e293b' }} />
