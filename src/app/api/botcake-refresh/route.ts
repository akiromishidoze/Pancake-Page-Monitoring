...
async function handler(apiKey: string | null, ip?: string) {
  ...
  try {
    if (!apiKey) {
      return addSecurityHeaders(cors(apiError(ErrorCodes.AUTH_REQUIRED, 'Missing X-Api-Key header', 401)));
    }
...
export async function GET(req: Request) {
  const apiKey = req.headers.get('x-api-key') || req.headers.get('X-Api-Key');
  if (!apiKey) {
    return addSecurityHeaders(cors(apiError(ErrorCodes.AUTH_REQUIRED, 'API key in URL (?key=) is no longer supported. Use POST with X-Api-Key header instead.', 400)));
  }
  return handler(apiKey, getClientIp(req));
}
